...
public class Futsal extends AbstractClass{
    ...
    public Futsal(int harga, int lama, String lapangan, String status, String kasir, String penyewa, String masuk, String keluar,String keterangan, java.sql.Date tglMain, java.sql.Date tglBooking){
        this.harga = harga;
        this.lama = lama;
    ...
    public void simpan( int DP,int total, int sisa){
        ...
        try{
            ...
            rs = st.executeQuery("SELECT tgl_main, masuk, keluar, lapangan FROM tb_booking_futsal WHERE tgl_main = '" + this.tglMain +"' and masuk ='" + this.masuk +"' and keluar ='"+ this.keluar+"' and lapangan='"+this.lapangan+"'");
            if(rs.next()){
               JOptionPane.showMessageDialog(null, "Data sudah ada , Dari jam "+rs.getString("masuk")+" - "+rs.getString("keluar")+", silahkan input data yang berbeda","Error", JOptionPane.ERROR_MESSAGE);
                ...
                st.executeUpdate("insert into tb_booking_futsal values('" +this.kdBooking+"', '"+ this.tglBooking+ "', '" +this.tglMain + "', '"+ 
    ...
    public void update(int DP, int total, int sisa,String kdBooking){
        ...
        try{
            st = cn.createStatement();
            st.executeUpdate("update tb_booking_futsal set tgl_booking = '" + this.tglBooking+ "',  tgl_main ='" + this.tglMain+ "', kasir = '"+ this.kasir+
    ...
    public void delete(String kdBooking) throws SQLException{
        st = cn.createStatement();
        st.executeUpdate("DELETE FROM tb_booking_futsal WHERE kd_booking='" + kdBooking + "'");
    }
